...
public class MainActivity extends ActionBarActivity {
    ...
    private void showCurrentLocation(Double latitude, Double longitude) {
       ...
       // Marker melbourne = map.addMarker(new MarkerOptions()
        ...
        LocationNode tmp_location = new LocationNode(latitude, longitude);
        location_list.add(tmp_location);

        if (location_list.size() != 1) {
            Polyline line = map.addPolyline(new PolylineOptions()
                    .add(new LatLng(location_list.getLast().GetLatitude(), location_list.getLast().GetLongtitude()),
                            new LatLng(location_list.get(location_list.size()-2).GetLatitude(), location_list.get(location_list.size()-2).GetLongtitude()))
                    .width(5)
                    .color(Color.RED));
